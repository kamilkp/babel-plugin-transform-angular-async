...
function functionExpression(path) {
  ...
  if (path.node.async) {
    ...
    if (_core.types.isFunctionDeclaration(path.node)) {
      exp = _core.types.functionExpression(path.node.id, path.node.params, path.node.body, path.node.generator, path.node.async);
    }

    const member = _core.types.memberExpression(exp, _core.types.identifier('call'));
    ...
    if (_core.types.isArrowFunctionExpression(path.node)) {
      ...
      expr = _core.types.functionDeclaration(path.node.id, arg, block);
    }
